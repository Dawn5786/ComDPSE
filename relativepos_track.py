...
# class AbsDiMPActor(BaseActor):
class RelativeDiMPActor(BaseActor):
    """Actor for training the DiMP network."""
    def __init__(self, net, objective, loss_weight=None):
        super().__init__(net, objective)
        if loss_weight is None:
    ...
    def __call__(self, data):
        ...
        target_scores, bb_scores = self.net(train_imgs=data['train_images'],
                                            ...
                                            train_X_pos_matrixs=data['train_Xpos_matrixs'],
                                            train_Y_pos_matrixs=data['train_Ypos_matrixs'],
                                            test_X_pos_matrixs=data['test_Xpos_matrixs'],
                                            test_Y_pos_matrixs=data['test_Ypos_matrixs'],
                                            train_anno_center=data['train_anno_center'],
                                            test_anno_center=data['test_anno_center'],
                                            test_proposals=data['test_proposals'])
        ...
        # Total loss
        loss = loss_bb_ce + loss_clf_ce + loss_clf_ce_init + loss_clf_ce_iter + \
                            loss_target_classifier + loss_test_init_clf + loss_test_iter_clf   # loss_clf_ce  loss_clf_ce_init  loss_clf_ce_iter  0?
