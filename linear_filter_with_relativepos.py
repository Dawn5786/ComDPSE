...
class LinearFilterWithRelativePos(nn.Module):
    ...
    def forward(self, train_feat, test_feat, train_label, test_label, train_X_pos_matrixs, train_Y_pos_matrixs,
                test_X_pos_matrixs, test_Y_pos_matrixs, train_anno_center, test_anno_center, train_bb, test_bb, *args, **kwargs):
        ...
        num_img_test = test_feat.shape[0]  #diff
        ...
        # diff: transformer_add
        # encoded_feat, decoded_feat = self.transformer(train_feat, test_feat, train_label)
        encoded_feat, decoded_feat = self.transformer(train_feat, test_feat, train_X_pos_matrixs, train_Y_pos_matrixs,
                test_X_pos_matrixs, test_Y_pos_matrixs, train_label, test_label, train_anno_center, test_anno_center, train_bb, test_bb)

        encoded_feat = encoded_feat.reshape(-1, num_sequences, *encoded_feat.shape[-3:]) # diff: -->train_feat
        decoded_feat = decoded_feat.reshape(-1, num_sequences, *decoded_feat.shape[-3:]) # diff: -->test_feat
